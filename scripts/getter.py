...
def get_course_dict(course_codes: list,
                    acadsem: tuple,
                    save_output: bool = True
                    ) -> dict:
    """Function that scrapes course site for course details
    and extracts it to a dictionary.

    Args:
        course_codes: List of course codes
        save_output: Whether to save the output as a pickle file
    ...
    for course_code in course_codes:
        ...
        for idx in range(0, len(course_df), row_interval):
            ...
            for row in course_timings_df.itertuples(index=False):
                ...
                weeks = parse_weeks(row.REMARK.lstrip(
                    'Teaching Wk') if isinstance(row.REMARK, str) else '1-13')
                day_map = {'MON': 0, 'TUE': 1, 'WED': 2,
                           'THU': 3, 'FRI': 4, 'SAT': 5}
                timeslots.append(
                    (day_map[row.DAY], start_time, end_time, weeks))

            course_dict[course_code].append(
                    CourseIndex(course_code=course_code,
                                index=course_index_number,
                                timeslots=timeslots))
    ...
    if save_output:
        ...
        filename = os.path.join("savefiles", filename)
