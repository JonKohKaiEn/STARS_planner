    ...
    for course_code in course_codes:
        ...
        for idx in range(0, len(course_df), row_interval):
            ...
            for row in course_timings_df.itertuples(index=False):
                ...
                weeks = parse_weeks(row.REMARK.lstrip('Teaching Wk'))
                for week in weeks:
                    timeslots.add((row.DAY, start_time, end_time, week))

            course_dict[course_code].append(CourseIndex(course_index_number, timeslots))
